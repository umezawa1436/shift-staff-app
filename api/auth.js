...
// ====== 共通ユーティリティ ======
function envOk() {
  return SUPABASE_URL && SERVICE_ROLE_KEY && SESSION_SECRET;
}
...
async function staffLogin(req, res, body) {
  ...
  if (deptId === undefined || deptId === null || !staffCode || !password) {
    return bad(res, 400, '入力が不足しています');
  }
  ...
  const accounts = await sb(
    `accounts?staff_code=eq.${codeNum}&dept_id=eq.${deptIdNum}&password_hash=eq.${hash}&role=eq.staff&select=id,name,dept_id,staff_id,staff_code,role`
  );

  if (!accounts || accounts.length === 0) {
    return bad(res, 401, 'パスワードが正しくありません');
  }
  const a = accounts[0];
  if (a.staff_id == null || a.staff_code == null) {
    // バックフィル前提だが、念のため
    return bad(res, 500, 'アカウント設定が不完全です。管理者に連絡してください');
  }
...
async function adminLogin(req, res, body) {
  ...
  const hash = await sha256(password);
  const accounts = await sb(
    `accounts?email=eq.${encodeURIComponent(email.toLowerCase())}&password_hash=eq.${hash}&select=id,name,email,role,dept_id`
  );
  if (!accounts || accounts.length === 0) {
    return bad(res, 401, 'メールアドレスまたはパスワードが正しくありません');
  }
  const a = accounts[0];
  if (a.role !== 'master' && a.role !== 'leader') {
    return bad(res, 403, '管理者権限がありません');
  }

  const token = createToken({ accountId: a.id, role: a.role, deptId: a.dept_id });
  return res.status(200).json({
    token,
    user: { id: a.id, name: a.name, email: a.email, role: a.role, deptId: a.dept_id },
  });
}
...
async function changePassword(req, res, body) {
  ...
  const check = await sb(
    `accounts?id=eq.${payload.accountId}&password_hash=eq.${currentHash}&select=id`
  );
  if (!check || check.length === 0) {
    return bad(res, 401, '現在のパスワードが正しくありません');
  }

  const newHash = await sha256(newPassword);
  await sb(`accounts?id=eq.${payload.accountId}`, {
    method: 'PATCH',
