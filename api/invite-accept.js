...
// 招待リンクからのアカウント作成API（非ログイン状態で呼ばれる）
//
// ハンドルする action:
//   - lookup : { token } → 招待情報とスタッフ情報を返す
//   - create : { token, email, password } → アカウント作成 or 既存アカウント更新
...
async function createFromInvite(req, res, body) {
  ...
  // このスタッフの既存アカウントを探す（同名+同部門）
  const sameStaffAccounts = await sb(
    `accounts?name=eq.${encodeURIComponent(staff.name)}&dept_id=eq.${staff.dept_id}&select=id,email`
  );
